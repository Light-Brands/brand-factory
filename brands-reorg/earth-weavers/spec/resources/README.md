# Resources

> Supporting documents and reference materials for Earth Weavers

---

## Overview

This folder contains reference documents, templates, and supporting materials for Earth Weavers planning and operations.

---

## Documents

| Document | Description |
|----------|-------------|
| [Original Thesis](./original-thesis.md) | The founding vision document |
| [Stakeholder Map](./stakeholder-map.md) | Key stakeholders and their interests |
| [Risk Register](./risk-register.md) | Comprehensive risk identification and mitigation |

---

## Quick Links

### Core Documents (in parent folders)
- [Main README](../README.md)
- [Council Assessment](../00-council-assessment.md)
- [Implementation Roadmap](../01-implementation-roadmap.md)

### Gates
- [Gate 1: Cultural Partnership](../gates/01-cultural-partnership.md)
- [Gate 2: Economic Sustainability](../gates/02-economic-sustainability.md)
- [Gate 3: Governance Simplification](../gates/03-governance-simplification.md)
- [Gate 4: Shadow Integration](../gates/04-shadow-integration.md)
- [Gate 5: Ecological Integrity](../gates/05-ecological-integrity.md)

### Phases
- [Phase 0: Foundation](../phases/00-foundation.md)
- [Phase 1: Pilot Design](../phases/01-pilot-design.md)
- [Phase 2: Alliance Building](../phases/02-alliance-building.md)

### Governance
- [Decision Framework](../governance/decision-framework.md)
- [Roles & Responsibilities](../governance/roles.md)
- [Council Integration](../governance/council-integration.md)

---

## Additional Resources (To Be Developed)

The following resources will be developed as Earth Weavers progresses:

| Resource | Purpose | When Needed |
|----------|---------|-------------|
| Brand Guidelines | Visual identity and voice | Phase 1 |
| Playbook | Complete operational guide | End of Phase 1 |
| Training Materials | Academy curriculum | Phase 2 |
| Partnership Templates | Standard agreements | Phase 1-2 |
| Impact Framework | Measurement methodology | Phase 1 |
| Technology Spec | Platform requirements | Phase 2 |

---

*"Resources serve the mission. Keep them organized, accessible, and current."*
