...
"Follow the instructions i../design/tools/cursor-prompt-base.txt"
```
...
AI Agent:
1. Rea../design/tools/cursor-prompt-base.txt
2. Rea../design/templates/component-template.tsx
3. Rea../design/guidelines.md
...
```bash
# Check a component
node tools/check-consistency.js components/CommunityCard.tsx

# Check a page
node tools/check-consistency.js app/community/discover/page.tsx

# Check a hook
node tools/check-consistency.js hooks/use-auth-state.ts

# Check an API route
node tools/check-consistency.js app/api/users/route.ts

# Check a utility
node tools/check-consistency.js utils/general-utils/format-date.ts
```
...
{
  "scripts": {
    "check-design": "node tools/check-consistency.js"
  }
}
```
...
for FILE in $STAGED_FILES; do
  echo "Checking $FILE..."
  node tools/check-consistency.js "$FILE"
...
1. **Before Starting:**
   ...
   cat tools/cursor-prompt-base.txt
...
2. **During Development:**
   ...
   node tools/check-consistency.js your-new-file.tsx
   ```
...
3. Use appropriate template fro../design/templates/
...
node tools/check-consistency.js components/*.tsx 2>&1 | tee compliance-report.txt
```
...
2. **Test New Check:**
   ...
   ```bash
   node tools/check-consistency.js test-file.tsx
   ```
...
- [CODEX Usage Guide](../codex-usage.md)
