...
// Calculate node positions in a uniform circular layout around the hub
function calculateNodePositions(): Record<string, { x: number; y: number }> {
  ...
  // Category groups arranged clockwise from upper-left
  const groups = [
    ['metro-1', 'future-of-cities', 'phx-jax', 'climate-hub'],       // Real Estate
    ['ccrl', 'chozen-ip'],                                             // Regenerative
    ['book-platform', 'course-platform', 'speaking-media', 'tony-cho-brand'], // Authority
    ['cho-foundation', 'ximena-cho-fund'],                             // Philanthropy
  ];

  const totalEntities = groups.reduce((sum, g) => sum + g.length, 0);
  const gapAngle = Math.PI / 12; // 15° gap between category groups
  const totalGap = gapAngle * groups.length;
  const entityAngle = (2 * Math.PI - totalGap) / totalEntities;
  let currentAngle = -(Math.PI * 3) / 4; // Start from upper-left

  groups.forEach((group) => {
    group.forEach((id) => {
      positions[id] = {
        x: centerX + Math.cos(currentAngle) * radius - 80,
        y: centerY + Math.sin(currentAngle) * radius - 40,
      };
      currentAngle += entityAngle;
    });
