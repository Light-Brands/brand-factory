...
- **Crypto Awareness**: Understands wallet state, VIBES balance, pending transactions, and rewards—explains crypto features in simple, approachable language
- **KARMA Awareness** (Epic 1B): Knows user's KARMA level, recent actions, planetary impact, and badges—celebrates recognition milestones and provides context on how positive actions earn KARMA
...
**Epic 1A - Crypto: Mira as Financial Guide**
```typescript
// Mira crypto context and prompts
interface CryptoPrompt {
  trigger: 'wallet_created' | 'reward_earned' | 'payment_received' | 'low_balance' | 'staking_mature';
...
interface WalletContext {
  handle: string;  // @username for payments
  vibeBalance: number;
  usdcBalance: number;
...
"Your wallet is ready! Anyone can send you VIBES or USDC using @${handle}. No wallet addresses needed."

// First VIBES reward earned:
"✨ You just earned 10 VIBES for completing your first practice! Keep showing up and watch your rewards grow."

// Payment received:
"Sarah just sent you $5. You can view the transaction in your wallet or send a thank-you message."

// Streak multiplier:
"Your 7-day streak just activated! You're now earning 1.5x VIBES on every practice. Keep going!"

// Staking explanation:
"Ready to unlock premium features? Staking 500 VIBES gives you priority matching and 2x voting power."
```

**Epic 1B - Karma: Mira as Recognition Celebrant**
```typescript
// Mira KARMA context and prompts
interface KarmaPrompt {
  trigger: 'karma_earned' | 'level_up' | 'badge_earned' | 'planetary_milestone' | 'weekly_summary';
  karmaContext: KarmaContext;
  message: string;
  celebration?: CelebrationLevel;
}

interface KarmaContext {
  totalKarma: number;
  level: number;
  levelName: string;          // "Flourishing", "Guardian", etc.
  vibesMultiplier: number;    // 1.0x to 2.0x based on level
...
// Examples:
// KARMA earned from practice:
"🧘 +5 KARMA for showing up! Your Guardian level gives you 1.75x VIBES on every practice."

// Level up celebration:
"🌟 You just reached Flourishing (Level 5)! Your 2,500 KARMA shows real commitment to growth. You now earn 1.25x VIBES on every action."

// Badge earned:
"✨ You've earned the Week Warrior badge! 7 days of consistency is where transformation begins. Want to mint this as a permanent on-chain achievement?"

// Planetary milestone:
"🌳 The VIBEUP community just planted our 10,000th tree! You've contributed 47 of those—a small grove! Your planetary KARMA: 2,350."

// Weekly summary:
"This week you earned 347 KARMA from 23 positive actions. You're #47 on the leaderboard! Keep helping the community to climb higher."

// Near level up:
"You're 153 KARMA away from Guardian level! One more week of consistent practice could get you there—and that 1.75x VIBES multiplier."
```
...
interface ComprehensiveUserContext {
  ...
  // Wallet & Crypto (Epic 1A) - READ ONLY
  // ⚠️ SECURITY: Mira can READ this data but CANNOT execute transactions
  wallet: {
    handle: string;              // @username for P2P payments
    isCreated: boolean;          // Whether wallet exists
    vibeBalance: number;         // Available VIBES tokens
    usdcBalance: number;         // Available USDC
    stakedBalance: number;       // Locked in staking pools
    pendingRewards: number;      // Unclaimed VIBES rewards
    lifetimeEarned: number;      // Total VIBES ever earned
    currentStakingTier?: 'explorer' | 'pathfinder' | 'founder';
    pendingPaymentRequests: number;
  };
  
  // KARMA & Recognition (Epic 1B)
  // Mira celebrates achievements and provides recognition context
  karma: {
    totalKarma: number;          // Lifetime KARMA score
    level: number;               // 1-10 level
    levelName: string;           // "Seedling", "Guardian", etc.
    vibesMultiplier: number;     // 1.0x to 2.0x based on level
    karmaThisWeek: number;       // Weekly KARMA earned
    karmaThisMonth: number;      // Monthly KARMA earned
    categoryBreakdown: {         // Where KARMA comes from
    ...
    recentActions: KarmaAction[];  // Last 10 KARMA-earning actions
    badges: Badge[];               // Earned badges
    weeklyRank: number;            // Leaderboard position
    nextMilestone: {               // Closest upcoming milestone
      type: 'level' | 'badge';
      name: string;
      current: number;
      required: number;
    ...
    recentTransactions: {
      type: 'reward' | 'transfer' | 'payment' | 'stake';
      amount: number;
      token: string;
      timestamp: Date;
    }[];
...
/**
 * ⚠️ MIRA CRYPTO SAFETY BOUNDARIES ⚠️
 * 
 * Mira uses CryptoQueryService (READ-ONLY) from @/lib/crypto/mira-safe
 * 
 * WHAT MIRA CAN DO:
 * ✅ Read wallet balances and history
 * ✅ Explain VIBES rewards and multipliers
 * ✅ Suggest sending money to someone ("Try the Pay button to send @alex $10")
 * ✅ Estimate transaction costs
 * ✅ Celebrate earnings and milestones
 * 
 * WHAT MIRA CANNOT DO:
 * ❌ Execute any transactions
 * ❌ Initiate verification flows
 * ❌ Access verification PINs
 * ❌ Approve or authorize payments
 * ❌ Stake or unstake VIBES
 * 
 * All execution requires out-of-band human verification (SMS/email PIN).
 * This is enforced architecturally - Mira's service cannot import execution code.
 */
```
...
- [`epic-1a-crypto.md`](epic-1a-crypto.md) - Crypto financial layer (Mira's wallet context)
- [`epic-02-humans.md`](epic-02-humans.md) - Profile data that Mira seeds
- [`crypto-router.md`](../architecture/crypto-router.md) - Crypto service architecture
- [`vibe-tokenomics.md`](../architecture/vibe-tokenomics.md) - VIBES token economics
