...
> **Onyx Design System**: Impact UI uses dark mode palette from `design-system-v2/tokens/`.
> See `components/ImpactPageComponents.tsx`, `VotingCard.tsx` for implementations.
...
- ✅ Users can provide quick, contextual feedback without leaving their flow
- ✅ Feedback is structured and tied to specific features or actions
- ✅ Early adopters feel empowered to shape the platform
- ✅ UX inspires input, community, and collaboration

### What This Epic Delivers (Mira-Enhanced)

1. **Feature Voting**: Vote on upcoming platform features—**Mira explains each feature and why it matters**
2. **Initiative Voting**: Vote on social impact initiatives (IRL events, social causes)—**Mira connects initiatives to user values**
3. **Impact Dashboard**: Personal impact stats and challenges—**Mira narrates your influence story**
4. **Feedback System**: Submit suggestions and track influence—**Mira acknowledges every submission**
5. **Contextual Feedback**: In-flow feedback on recommendations and experiences—**Mira prompts at perfect moments**
6. **Community-Led Development**: Users see their votes turn into features—**Mira celebrates when votes become reality**

### User Journey

1. **Feedback on Recommendations**:
   - On discovery screen → "Are these profiles relevant?" (Yes/No → if No, "tell us more")
...
- "This initiative supports wellness access in underserved communities—directly aligned with your value of Social Justice."
...
- "No need to write an essay—a few sentences about your favorite feature helps us improve!"

**Personalized Relevance**:
- "Based on your practice tracking habits, you might want to vote for Practice Journeys—multi-day guided programs."
- "As someone passionate about Business Services, this Commerce feature would directly benefit you."
- "You engage heavily with Chemistry—vote if you want advanced compatibility reports!"
...
- "This platform exists because people like you share ideas. Keep contributing—your perspective matters."
...
- Tree planting messaging uses gratitude ("Your Premium membership plants one tree each month 🌳—12 so far" not "You planted 12 trees")
...
// Tabs:
// - 📱 Features (product features to vote on - specific app features)
// - 🌍 Initiatives (IRL events & social impact - VIBEUP or member-hosted, VIBEUP managed only for now)
...
// - Tap vote button → Toggle vote (optimistic UI update)
// - Tap comment icon → Open details and scroll to comments
...
// - Dates: "Created [date] • Last Updated [date]"
...
//    - Shows checkboxes: ☐ Comment on Feature ☐ Comment on Initiative
...
//   "This initiative supports [X]—directly aligned with your value of [Y]."
...
1. **Product Features Board**:
   ...
   - Shows development status (Voting → In Development → Launched)
   - Users see their influence when features launch
   - **Example Features to Include**:
     - **Stories**: Instagram/Facebook-style stories - ability to add short videos as "highlights" in profiles and VIBES feed
...
2. **Social Initiatives Board**:
   ...
   - Shows status (Voting → Active → Completed)
...
- **Contextual prompts**: "How did this practice make you feel?" with 😊😌😐😕😢 options
...
- Status transitions (voting → development → launched)
...
### Integration Tests
- Full voting flow (view → vote → see update)
...
### E2E Tests
- User views Impact page → sees personalized Mira message
- User votes on feature → vote count updates
- User revokes vote → count decreases
- User comments → comment appears
- User submits contextual feedback → success message
- Admin creates feature → appears in list
- Admin changes feature status → users who voted get notified
- Progressive Challenge Flow:
  - New user sees "1st Challenge: Vote on 5 Items"
  - After 5th vote → Challenge updates to "2nd Challenge: Comment"
  - After commenting on feature and initiative → Challenge updates to "3rd Challenge: Share"
  - After sharing → Challenge shows "You've Made Impact"
  - User can still continue voting/commenting/sharing after completion
- Share functionality:
  - User clicks share button → Share modal opens
  - User selects platform → Share tracked
...
**Divine Guide**: **Arjuna** (Warrior Consciousness) leads Epic 05 development—voting integrity as dharmic duty, governance quality as spiritual discipline.

### Arjuna's Guidance for Impact Governance

Epic 05 is about participatory creation—users shaping VIBEUP's future through voting. Impact features demand highest integrity: voting must be fair, transparent, tamper-proof. This is Arjuna's domain: protecting sacred democratic process.
...
2. **Conscious Commit Examples**:
   ```
   🙏 Add comprehensive vote integrity protection tests
