...
> **Onyx Design System**: Monetization UI uses dark mode palette from `design-system-v2/tokens/`.
> Premium features use `gold-accent` (#D4AF37) for visual distinction.
...
export class MonetizationService extends BaseDatabaseService {
  ...
  private generateAffiliateCode(): string {
    // Generate unique 8-character code (e.g., "VIBEUP23")
    return 'VIBE' + Math.random().toString(36).substring(2, 8).toUpperCase();
  }
