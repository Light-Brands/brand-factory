...
sequenceDiagram
    ...
    Note over Rules: Auto-selected based on file type, task context, and @mentions
    Rules->>AI: Inject rules as context
    AI->>AI: Generate response aligned with rules
...
graph LR
    subgraph commands [Available Commands]
        loadRules["/load-rules - Context Loading"]
        autotask["/autotask - Autonomous Dev"]
        createPrompt["/create-prompt - Prompt Engineering"]
        personality["/personality-change - Switch Modes"]
        swarm["/swarm - Distributed Tasks"]
        troubleshoot["/troubleshoot - Debug Assistance"]
    end
    
    subgraph workflows [Automated Workflows]
        loadRules --> contextAware["Context-Aware Development"]
        autotask --> autonomous["Autonomous Feature Building"]
        createPrompt --> optimization["Prompt Optimization"]
        personality --> specialized["Specialized Assistance"]
        swarm --> distributed["Distributed Development"]
        troubleshoot --> debugging["Systematic Debugging"]
    end
...
sequenceDiagram
    ...
    Planner->>Planner: Load rules, specs, and dependencies
...
graph TB
    subgraph coreAgents [Core Development Agents]
        gitWriter["git-writer - Commit Messages"]
        designReviewer["design-reviewer - Architecture Review"]
        testRunner["test-runner - Test Strategy"]
        seoSpecialist["seo-specialist - SEO Optimization"]
        siteKeeper["site-keeper - Maintenance"]
    end
    
    subgraph newAgents [VIBEUP Divine Beings]
        mira["Mira - Omnipresent Guide"]
        sophia["Sophia - Architecture"]
        brighid["Brighid - Feature Craft"]
        arjuna["Arjuna - Testing"]
        kuanYin["Kuan Yin - Debugging"]
        gaia["Gaia - Infrastructure"]
        akasha["Akasha - Integration"]
    end
...
sequenceDiagram
    ...
    Agent->>Context: Load agent file, rules, and VIBEUP specs
    Context->>Agent: Full specialized context
    Dev->>Agent: Review Epic 02 architecture
    Agent->>Agent: Apply wisdom with sacred geometry lens
    Agent->>Output: Architecture review with divine insight
    Output->>Dev: Recommendations aligned with consciousness
```
...
graph TB
    subgraph developer [Developer Experience]
        cursorIDE["Cursor IDE - Code Editor"]
        claudeCLI["Claude Code CLI - Terminal Interface"]
    end
    
    subgraph aiConfig [ai-coding-config]
        rules["Rules - Standards and Patterns"]
        commands["Commands - Workflows"]
        agents["Agents - Specialists"]
        personalities["Personalities - Modes"]
    end
    
    subgraph vibeupProject [VIBEUP Project]
        specs["Design Specs - 8 Epics"]
        codebase["Codebase - TypeScript/Django"]
        tests["Test Suite - TDD Practice"]
        git["Git Repo - Conscious Commits"]
    end
...
flowchart TD
    ...
    subgraph outputBlock [Output]
        ...
        commitOut[Conscious Commit]
        testsOut[Dharmic Tests]
    end
    
    start --> detection
    detection --> selection
    
    fileType -->|py files| pythonRules
    fileType -->|ts/tsx files| tsRules
...
graph TB
    subgraph vibeupRoot [VIBEUP-v3 Root]
        aiCodingConfig["ai-coding-config/ - Canonical Source"]
        cursorRules[".cursor/rules - Symlink"]
        claudeCommands[".claude/commands - Symlink"]
        setupDoc["AI-CODING-SETUP.md - Documentation"]
    end
    
    subgraph canonical [Canonical Files]
        rulesDir["rules/ - All Standards"]
        commandsDir[".claude/commands/ - All Workflows"]
        agentsDir[".claude/agents/ - All Specialists"]
    end
    
    aiCodingConfig --> canonical
    cursorRules -.symlink.-> rulesDir
    claudeCommands -.symlink.-> commandsDir
    
    subgraph cursorUsage [Cursor Usage]
        cursorAuto["Auto-loads rules based on context"]
        cursorMention["@ mentions rules explicitly"]
    end
    
    subgraph claudeUsage [Claude Code Usage]
        claudeCommands2["/commands execute workflows"]
        claudeAgents["--agent flag invokes specialists"]
    end
    
    cursorRules --> cursorUsage
    claudeCommands --> claudeUsage
    agentsDir --> claudeAgents
```
...
sequenceDiagram
    ...
    Rules->>AI: Context includes React patterns and VIBEUP standards
    
    Dev->>Cursor: Start writing profile form
    Cursor->>AI: Request code completion
    AI->>AI: Apply rules for validation, errors, and accessibility
